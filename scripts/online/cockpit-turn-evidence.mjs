...
let stage = 'setup', pageErrors = 0, unexpectedConsole = 0;
const rows = Array.from({ length: 40 }, (_, i) => ({
  ...
  card: {
    ...
    printedName: `進行検証${i}`,
    lang: 'ja', layout: 'normal', cmc: 0, colorIdentity: [], typeLine: 'Creature',
    faces: [{ name: `Turn Creature ${i}`, printedName: `進行検証${i}`, typeLine: 'Creature',
      manaCost: '{0}', oracleText: '', power: '2', toughness: '2' }],
  },
}));
async function page() {
  ...
  p.on('console', (m) => {
    if (m.type() === 'error' && !(m.text().startsWith('Failed to load resource:') && m.text().includes('409')))
      unexpectedConsole++;
  });
  await p.goto(origin);
...
async function menu(p) {
  ...
  if (!(await m.isVisible())) {
    ...
    await ((await b.isVisible()) ? b : p.getByRole('button', { name: 'メニュー', exact: true })).click();
  }
  return m;
}
...
async function keep(p) {
  for (let tries = 0; tries < 3; tries++) {
    ...
    await until(async () => {
      ...
      return v.table.seats.find((s) => s.id === (v.multiplayer?.ownSeatId ?? 'P1')).kept ||
...
async function mutate(p, perform, label, attempt = 0) {
  ...
  const wait = p.waitForResponse((r) => r.url().includes('/api/cockpit/') &&
    r.request().method() === 'POST' && ['commit', 'control'].includes(r.request().postDataJSON()?.type));
  await perform();
  const r = await wait;
  const v = await r.json();
  if (r.status() !== 200) {
    assert.equal(v.error, 'REVISION_CONFLICT', label);
    await p.locator('.table-connection').getByRole('button', { name: '閉じる', exact: true }).click();
...
try {
  ...
  const invitation = await (await menu(host)).getByLabel('招待コード', { exact: true }).inputValue();
  ...
  await keep(host); await keep(guest);
  await menu(host);
  await mutate(host, () => host.getByRole('button', { name: '全員で対戦を開始', exact: true }).click(), 'start');
  await closeMenu(host);
  ...
  report.checks.push({ name: 'two-player first draw skipped through normal buttons and key', passed: true });
  ...
  await mutate(host, () => host.getByRole('button', { name: '支払って唱える', exact: true }).click(), 'cast');
  await mutate(host, () => host.getByRole('button', { name: '解決', exact: true }).click(), 'resolve');
  ...
  await mutate(host, () => details.getByRole('button', { name: '修整を追加', exact: true }).click(), 'modifier');
  ...
  await mutate(host, () => host.getByRole('button', { name: 'Gマナを追加', exact: true }).click(), 'mana');
  ...
  for (let round = 0; round < 4; round++) {
    ...
    if (round > 0) {
      await menu(host);
      await mutate(host, () => host.getByRole('button', { name: '部屋主が操作権を回収', exact: true }).click(), 'reclaim');
      await closeMenu(host);
      if (active === 'P2') {
        await menu(guest);
        await mutate(guest, () => guest.getByRole('button', { name: 'HOLD・応答を要求', exact: true }).click(), 'hold');
        await closeMenu(guest);
        await menu(host);
        await mutate(host, () => host.getByRole('button', { name: '操作権を貸す', exact: true }).click(), 'grant');
        await closeMenu(host);
      }
      await until(async () => Boolean((await read(actor)).multiplayer.canOperate), 'control visible');
      ...
      await next(actor);
      assert.equal((await read(actor)).table.seats.find((s) => s.id === active).zones.hand.length, before + 1);
      if (active === 'P1') assert.equal((await read(actor)).table.cards[permanent.id].tapped, false);
    }
    state = await read(actor);
    while (state.table.phase !== 'cleanup') state = await next(actor, state.table.phase === 'main2');
    ...
    if (!state.table.cleanupReady) {
      ...
      for (let i = 0; i < required; i++) await cleanup.getByRole('checkbox', { name: /捨てる：/ }).nth(i).check();
      await mutate(actor, () => cleanup.getByRole('button', { name: 'クリーンナップを完了', exact: true }).click(), 'complete cleanup');
      await cleanup.waitFor({ state: 'hidden' });
      assert.equal((await read(actor)).table.seats.find((s) => s.id === active).zones.hand.length, 7);
    }
  ...
  report.checks.push({ name: 'two full rounds: untap, one normal draw, cleanup discard, mana/effect expiry, draw-step reload and modal key lock', passed: true, turns: totalTurns });
