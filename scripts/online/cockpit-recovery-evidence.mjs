...
async function apiProbe() {
  ...
  const second = await call(p2, {
    type: 'join',
    deck: deck(true),
    invitation: first.multiplayer.invitation,
  });
  const change = async (actor, type, operation) => {
    const view = await call(actor, { type: 'read' });
    return call(actor, {
      type,
      [type === 'control' ? 'control' : 'operation']: operation,
  ...
  await change(p2, 'commit', {
    ...
    paymentPlan: [
      { type: 'payMana', payment: { W: 0, U: 0, B: 0, R: 0, G: 0, C: 0 }, playerId: 'P2' },
    ],
  });
  for (const operation of [
    null,
    { type: 'resolve.begin' },
    { type: 'move', ids: [sourceId], to: 'hand', position: 'top' },
  ]) {
  ...
  record(
    'real Worker/SQLite: hidden source protected through activation, resolution, and source movement',
  );
}
...
try {
  ...
  if (!process.argv.includes('--api-only')) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      page.on('console', (message) => {
        if (
          message.type() === 'error' &&
          !(expectedNetworkFailure && message.text().startsWith('Failed to load resource:'))
        )
      ...
      await page.evaluate(async (rows) => {
        ...
        await saveResolvedDeck({
          deckText: 'Stage 1 local acceptance fixture',
    ...
    for (const page of pages) await page.getByTestId('mulligan-keep').click();
    await menu(host);
      ...
      if (body.type === 'commit' && body.operation?.type === 'draw') {
        ...
        if (fault === 'reject') {
          ...
          await route.continue({
            postData: JSON.stringify({ ...body, operation: { ...body.operation, count: 500 } }),
          });
          return;
        }
        if (fault === 'lost') {
    ...
    record(
      'actual 422 rejection leaves board unchanged; next normal keyboard draw succeeds without reconnect',
    );
    ...
    await draw(host);
    await waitUntil(
      async () => (await hand(host)) === before + 2,
      'draw after automatic read recovery',
    );
    ...
    record(
      'real saved draw with lost response: receipt reconciliation, no replay and one-card delta',
    );
    ...
    const original = await host.evaluate(
      () => JSON.parse(localStorage.getItem('mtg-onedeck:cockpit-connection-v1')).id,
    );
    ...
    const retained = await host.evaluate(
      () => JSON.parse(localStorage.getItem('mtg-onedeck:cockpit-connection-v1')).id,
    );
    ...
    record(
      'failed join to a started room, screen disposal and reload restore original seat and exact board',
    );
    ...
    record(
      'three viewports, zero page exceptions and zero unexpected console errors; injected network errors excluded explicitly',
    );
  }
  report.passed = true;
} catch (error) {
  ...
  // Do not print Playwright call logs, which could contain a synthetic invitation or session URL.
  process.exitCode = 1;
} finally {
