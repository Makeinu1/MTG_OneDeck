...
export function renderProjectState({ index, capabilities }) {
  ...
  const lines = [
    ...
    `- Verification integrity: ${mdCode(index.roles.verificationIntegrity)}`,
    `- Verification impact resolver: ${mdCode(index.roles.verificationImpactResolver)}`,
    `- Candidate verification gate: ${mdCode(index.roles.candidateVerificationGate)}`,
    `- Work Protocol: ${mdCode(index.roles.workProtocol)}`,
    `- Work Order schema: ${mdCode(index.roles.workOrderSchema)}`,
    `- Work Order validator: ${mdCode(index.roles.workOrderValidator)}`,
    `- Work Protocol integrity: ${mdCode(index.roles.workProtocolIntegrity)}`, '',
