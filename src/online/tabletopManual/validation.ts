...
export function validateOnlineTabletopIntentEnvelopeV1(input: unknown): OnlineTabletopIntentValidationResultV1 {
  ...
  const primitiveFields = ['kind', 'objectId', 'targetObjectId', 'destination', 'zone', 'order', 'count', 'tapped', 'counterKind', 'color', 'field', 'delta', 'gainingControllerPlayerId', 'amount', 'noteId', 'entryId', 'text', 'label', 'sourceObjectId', 'tokenSeed', 'definitionId', 'definition', 'held', 'actionsWereApplied'] as const;
  ...
  if (primitive !== null) {
    ...
    const allowed = allowedByKind[kind]; if (allowed === undefined) issues.push(issue('INVALID_LITERAL', '/primitive/kind', 'Unsupported tabletop primitive')); else {
      ...
      if (kind === 'sba-check-outcome' && primitive.actionsWereApplied !== undefined && typeof primitive.actionsWereApplied !== 'boolean') issues.push(issue('INVALID_TYPE', '/primitive/actionsWereApplied', 'SBA outcome must be boolean'));
