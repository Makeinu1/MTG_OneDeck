...
function request(body: unknown): Request {
  return new Request('http://localhost/api/cockpit/private-library', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
}
function room() {
  const storage = database(); let revision = 0; let now = 1000;
  const credentials = [
  ...
  async function call(index: number, body: Record<string, unknown>) {
    const response = await handleCockpitSession(request({ ...credentials[index], ...body }), storage, now++);
  ...
  async function change(index: number, operation: Record<string, unknown>, control = false) {
    return call(index, { type: control ? 'control' : 'commit', requestId: crypto.randomUUID(), revision, [control ? 'control' : 'operation']: operation });
  }
  async function start() {
    ...
    expect((await call(1, { type: 'join', invitation, deck: makeDeck(20) })).response.status).toBe(200);
...
it('projects only requested top-N cards, never to another seat, and full browse remains distinct', async () => {
  ...
  const bounded = await shared.change(0, { type: 'peek', seatId: 'P1', zone: 'library', count: 2 }, true);
  ...
  const invalid = await shared.change(0, { type: 'peek', seatId: 'P1', zone: 'hand', count: 2 }, true);
...
it('revokes private library projection when control changes and suppresses it while owner is absent', async () => {
  ...
  const p2Peek = await shared.change(1, { type: 'peek', seatId: 'P2', zone: 'library', count: 2 }, true);
