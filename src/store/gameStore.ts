...
export const useGameStore = create<GameStore>((set, get) => {
  ...
  return {
    ...
    confirmGuidedDiscard(cardId) {
      ...
      // below collapses to the original `!cur.zones.hand.includes(cardId)` check unchanged.
      const alreadyDiscarded = variableLoot ? variableLootDiscardedCardIds(pending.commands) : [];
      if (!cur.zones.hand.includes(cardId) || alreadyDiscarded.includes(cardId)) {
        set({
      ...
      const remainingHand = cur.zones.hand.length - alreadyDiscarded.length - 1;
      ...
      if (reachedMax || remainingHand <= 0) {
        ...
        advanceGuidedResolution([...discardCommands, { type: 'draw', count: drawCount }]);
        return;
      }
    ...
    cancelGuidedPrompt() {
      ...
      if (variableLoot) {
        ...
        advanceGuidedResolution([{ type: 'draw', count: drawCount }]);
        return;
      }
      advanceGuidedResolution([]);
    },
