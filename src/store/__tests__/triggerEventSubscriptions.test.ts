...
describe('event-log trigger subscriptions', () => {
  ...
  it('creates a pending trigger from a DamageEvent subscription', () => {
    ...
    store().dispatch({ type: 'dealDamage', sourceId, amount: 1, combatDamage: false, targetPlayerId: 'OPPONENT_A' });
    expect(pendingFor(sourceId)).toMatchObject([{ sourceId, triggerId: 'trigger.damage', label: 'ダメージを与えたとき: 《損傷の学者》', stackPlacementBucket: 'ordinary' }]);
  ...
  it('creates a pending trigger from a DrawEvent subscription without implicit duplication', () => {
    ...
    expect(pendingFor(sourceId)[0]).toMatchObject({ triggerId: 'trigger.draw', label: 'カードを引いたとき: 《ドローの観測者》' });
  });
  ...
  it('creates a pending trigger from a LifeChangeEvent subscription', () => {
    ...
    expect(pendingFor(sourceId)).toMatchObject([{ triggerId: 'trigger.life-gain', label: 'ライフを得たとき: 《生命の観測者》' }]);
  });
  ...
  it('creates an ETB pending trigger from a ZoneChangeEvent to battlefield', () => {
    ...
    store().moveCard(sourceId, 'battlefield');
    expect(pendingFor(sourceId)).toMatchObject([{ triggerId: 'trigger.etb', eventId: snap().eventLog.at(-1)?.eventId, label: '戦場に出たとき: 《悪意の大梟》' }]);
  });
  ...
  it('restoreGame backfills the once-per-turn trigger ledger for legacy snapshots', () => {
    ...
    const snapshot: GameSnapshot = { version: SNAPSHOT_VERSION, state: legacyState as GameState, deck, autoAdvanceToMain: store().autoAdvanceToMain };
