...
it('distinguishes full library access from bounded top-card access', () => {
  ...
  expect(
    hasCockpitLibraryAccess(access({ seatId: 'P1', zone: 'library', count: 2 }), 2),
  ).toBe(true);
  expect(
    hasCockpitLibraryAccess(access({ seatId: 'P1', zone: 'library', count: 2 }), 3),
  ).toBe(false);
  expect(
    hasCockpitLibraryAccess(access({ seatId: 'P1', zone: 'library', count: 2 })),
  ).toBe(false);
...
it('treats a confirmed bounded peek as sufficient when the real library is shorter', () => {
  expect(
    hasCockpitLibraryAccess(access({ seatId: 'P1', zone: 'library', count: 5 }, 1), 5),
  ).toBe(true);
});
