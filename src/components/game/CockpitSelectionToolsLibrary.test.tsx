...
function projections(count = 2) {
  ...
  const multi: CockpitMultiplayer = {
    invitation: 'selection-private-library',
    started: true,
    masterId: 'P1',
    holds: [],
    borrowedFrom: null,
  ...
  return {
    hidden: hidden.table,
    bounded: bounded.table,
    total: hidden.multiplayer.counts.P1.library,
    peek: bounded.multiplayer.peek!,
  };
}
...
function mount(table: CockpitTable, access: CockpitLibraryAccess) {
  ...
  const send = vi.fn<React.ComponentProps<typeof CockpitSelectionTools>['send']>(() =>
...
it('acquires only requested top cards before milling and commits those confirmed ids', async () => {
  ...
  const request = vi.fn((count?: number) => {
    expect(count).toBe(2);
...
it('invalidates an arranged private draft on same-length reorder and hides it after access loss', () => {
  ...
  try {
    ...
    [reordered.seats[0].zones.library[0], reordered.seats[0].zones.library[1]] = [
      reordered.seats[0].zones.library[1],
      reordered.seats[0].zones.library[0],
    ];
