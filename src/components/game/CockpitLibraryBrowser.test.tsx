...
function fixture() {
  ...
  const multi: CockpitMultiplayer = {
    invitation: 'browser-private-library',
    started: true,
    masterId: 'P1',
    holds: [],
    borrowedFrom: null,
...
it('general library entry distinguishes unrequested from empty and can start a private full browse', async () => {
  ...
  const render = (projection: typeof hidden) =>
    act(() =>
      root.render(
        <CockpitTableSurface
          view={{
            table: projection.table,
            multiplayer: projection.multiplayer,
            canUndo: false,
            canRedo: false,
          }}
          disabled={false}
  ...
  try {
    ...
    expect(host.textContent).toContain(
      `未取得です。山札は${hidden.multiplayer.counts.P1.library}枚あります。`,
    );
    expect(host.textContent).toContain('取得前の0件表示は候補0件を意味しません');
    const open = [...host.querySelectorAll<HTMLButtonElement>('button')].find((entry) =>
      entry.textContent?.includes('この山札を自分だけ閲覧'),
    )!;
    await act(async () => {
      open.click();
