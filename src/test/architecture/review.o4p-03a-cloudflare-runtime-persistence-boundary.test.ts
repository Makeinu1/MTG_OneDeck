    ...
    for (const filePath of productionFiles(cloudflareRoot)) {
      ...
      for (const specifier of moduleSpecifiers(sourceText)) {
        ...
        const cockpitR4AuthorityImport =
          normalized(filePath) === 'src/online/cloudflare/cockpitR4Authority.ts' &&
          ['../../engine/cockpitTable', '../../engine/commands', '../../engine/cockpitR4'].includes(specifier);
        const cockpitR4bAuditImport =
          normalized(filePath) === 'src/online/cloudflare/cockpitR4bAudit.ts' &&
          ['../../engine/cockpitTable', '../../engine/cockpitR4b'].includes(specifier);
        const cockpitR4bSessionImport =
          normalized(filePath) === 'src/online/cloudflare/cockpitR4bSession.ts' &&
