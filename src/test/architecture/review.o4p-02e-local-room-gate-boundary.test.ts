...
describe('O4P-02E local headless room gate architecture boundary', () => {
  ...
  it('prevents reverse dependencies and limits src/online to the registered module kinds', () => {
    ...
    const onlineKinds = readdirSync(onlineRoot, { withFileTypes: true })
      ...
      .map(({ name }) => name)
      .sort();
    expect(onlineKinds).toEqual(['architecture', 'bootstrap', 'browser', 'cloudflare', 'deckSubmission', 'displayPairing', 'genesis', 'guidedActions', 'headless', 'lobby', 'projection', 'protocol', 'publicApp', 'room', 'tableDisplay', 'workbench']);
  });
});
