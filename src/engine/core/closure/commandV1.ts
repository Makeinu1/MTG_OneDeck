...
export function validateCoreCommandV1(input: unknown): CoreCommandValidationResultV1 {
  ...
  if (payloadRecord) {
    ...
    } else if (kind === 'table-priority-hold') {
      const row = requireRecord(['kind', 'held']); if (row) { if (typeof row.held !== 'boolean') issues.push(issue('INVALID_TYPE', '/payload/held', 'HOLD state must be boolean')); payload = Object.freeze({ kind, held: row.held as boolean }); }
    ...
    } else if (kind === 'table-turn-progress' || kind === 'table-turn-progress-v2') {
