...
可聴範囲はゲーム画面かつ初回gesture解除済みとし、BGMはテーマの有効Track、ゲーム進行音は保存設定へ従う。
現在はダーク・ライト双方に選択済みTrackがあり、ライトでも既存の意味イベントSEを同じミックスで再生する。旧D7の「ループBGMを作らない」「UI/カード/状態の3レイヤー」は撤回し、次のbusへ置換する。
...
- **ライト = 墨の世界(ペースアンカー: 液态呼吸 3400ms)**。AV5/AV6のパーマネント/カード/統率者/照明振付もダークと同じ既存時計で有効にし、表現はライト用トークンで落ち着かせる:
