...
**現在値**(割当更新はこの行だけ編集): 判定者・実装者・冷監査者すべて ChatGPT で担当可(親タスク=判定者、別タスク=実装者、実装文脈を持たない別タスク=冷監査者)。Claude を使う場合の起動は `CLAUDE.md` 経由。反復手順の正本 = `.agents/skills/mtg-onedeck-development/`。
