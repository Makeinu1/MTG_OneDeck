...
- **旧UI 死CSS の残債(2026-07-19 発生・D4 App.css purge で回収)**: 旧 Playmat + 12ファイル削除により、`App.css` の `.playmat*`・`.playmat__handrow`・`.hand__*`・`.stack__*`(=`.stack-band`/`.stack-workspace` は新UI現用ゆえ別物・温存)・`.mobile-zone-swap*`・`.mobile-controls-drawer*`・`.battlefield*` が **~580行(71ブロック超)まとめて死CSS化**した(新UIは `hand-ribbon__*`/`stack-band`/`stack-workspace__*` の別名前空間を使う)。今回のコミットでは直接結合していた `.playmat-shell`/`.rotate-notice*` のみ除去し、この大区画は**別スライスへ分離**(共有され得る `.zones`/`.card-view--hand`/`.game-log` 等の個別到達性解析を要するため)。`review.d0-dead-css-scan.test.ts` の pin はこの一掃時に拡張する。
