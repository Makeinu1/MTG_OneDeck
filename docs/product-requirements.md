...
2026-09-09のユーザー合意により、最初にv0.41のCockpitへ既存の一人回しを接続し、
実デッキを読み込んで遊べる画面として公開する。統率者演出、手動操作、選択と履歴などの
既存の良い機能を継承する。その後、同じ盤面と意味モデルで2人対戦、4人対戦へ拡張する。
一人回しの公開は、共有sessionの信頼性や2人・4人の完全試合の証明を代替しない。
...
MVPは大きなPC画面で使うCockpitを対象とする。2026-09-08のproduct owner判断により、
小さいbrowser画面・モバイル向けのlayoutとresponsive検証はMVPの対象外とする。
代表検証viewportは1440×900とし、この画面での盤面、操作、詳細表示、復帰の作り込みを優先する。
ブラウザ上のオンライン対戦そのものは引き続き対象である。
