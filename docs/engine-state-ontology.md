...
誘発型能力(CR603)・状況起因処理(SBA, CR704)・優先権直前処理(CR117.5)・ターンベース処理(CR703)。これらが読む state は特性ではなく
**「直前に何が起きたか」=イベント**である。エンジンは盤面遷移を**イベントとして発行**し、誘発の観測者へ配る
(CR603.3)。将来の runtime ではこれを `emitEvent(state, event)` → `collectTriggers(state, event)` → `pendingTriggers`
(`src/engine/events.ts`、S-EVENTS で実装)が担う。誘発はイベント直後に即スタック化せず、CR117.5/704.3 の
`stabilizeBeforePriority()` で SBA と pending trigger placement を固定点まで処理してから優先権へ進む。
**本スライスはそのために必要なイベント軸を CR とコーパスから確定する。**
...
- **CR根拠**: 603.2(`When/Whenever/At` の誘発型能力)、603.3(誘発後、次に優先権を得る前にスタックへ)、603.3b(複数誘発のAPNAP/選択順)、603.8(状況誘発=条件が真の間に一度)、704.3/117.5(SBA/誘発/優先権ループ)
- **コーパス需要**: `Whenever` ≈ 4,904 / `When` ≈ 4,356 / `At the beginning of …` ≈ 1,677(粗概算)
- **state read/write**: read = 発行済イベント列 + 現ステップ/フェイズ + `pendingTriggers`。誘発は効果 state を**直接書かない**。イベント観測時は `pendingTriggers` に入れ、優先権前処理でスタックへ誘発オブジェクトを積む。
- **検証手段**: ゴールド `review.event-coverage` + コーパス頻度 + LLM-oracle 盲予測(別主体)
- **trust**: 未検証(下面抽出前)
- **既知欠落**: 反射誘発(reflexive, CR603.12)・遅延誘発(delayed, CR603.7)は概念登録のみ。本スライスは「直近イベント→pending trigger」の素形に限定
...
> **read-timing / object-identity を第一級に持つ**(architecture §2.1)。CR400.7 により、領域を移動したオブジェクトは原則として新オブジェクトになる。
> したがって実装モデルは `physicalCardId`(実カード/トークン由来の表示・所有)と `objectId` または `zoneChangeCounter`
> (現在の領域 incarnation)を分ける。「どのゾーンの・どの時点の・誰の」オブジェクトを読むかは値ではなく
> 構造であり、Slice3 はこの構造軸をコーパス需要で裏取りする。LKI は本スライスでは概念登録に留めるが、
> death/LTB/公開ゾーン移動 gold では event before/after snapshot として検査する。
