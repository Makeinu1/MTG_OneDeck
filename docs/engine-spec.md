...
> **現況(2026-06-25・M-GATE-4 完了)= FROZEN**(全7条件 PASS・`research/m-contract-gate/scorecard.md`)。
> ゲート・スコアカード器(§34.7.2)で全7条件を再現可能数値に確定し、M-GATE-2/3/4 で残条件を緑化した。
...
>
> **凍結到達 → 次 = M-FREEZE**(§34 draft の凍結手続き → S-EVENTS 実装着手)。M-FREEZE までは本節の draft 状態を維持する。
...
`research/m-contract-gate/scorecard.{md,json}` 再生成。条件3=PASS(churn<5%)・条件4=PASS・総合 **FROZEN** を確認。
FROZEN 確認後に §34.7.1 現況ブロックを更新(全7緑 → M-FREEZE へ)。**§34 本体の凍結 draft 解除は別 MS(M-FREEZE)**。
...
- **M-GATE-4(条件4+条件3 緑化 → 凍結到達・2026-06-25・監査合格)= 手法 §34.7.5**: 条件4(非LLM独立物差し)を散文監査から**機械可読 CR 真理テーブル**へ昇格。
  ...
  **条件3 post-yardstick churn**: CR 修正適用後に 4スライス同時スナップショット(baseline=`04184e4`)= layer 0%/event 1.95%/zone 0%/timing 0.82% → max **1.95% < 5% PASS**。
  ...
  全7条件 PASS=**スコアカード FROZEN**。`src/engine/` 盤面挙動不変・機械4点緑・`review.cr-conformance` 緑。**次 = M-FREEZE(§34 凍結手続き → S-EVENTS)**。
