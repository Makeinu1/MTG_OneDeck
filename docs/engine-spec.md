...
**判定者裁定が要る残フォーク(実装ブリーフ確定前)**: (A)linkId 生成源=解決中 effect/ability の安定 id(J2 暫定=effect 単位束ね・CR406.6 と一致)。(B)delayed-trigger の「次の終了ステップ」スケジュール表現可否=実装前に Explore で `'delayed-triggered'` 機構を裏取り、不可なら本スライスは same-resolution blink のみに絞り end-step blink を defer。(C)record GC=当面しない(stale は objectId 不一致で無効化)。(D)戻し control=owner 固定(controller 指定は defer)。
