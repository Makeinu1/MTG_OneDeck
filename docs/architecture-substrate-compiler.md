...
> 最終更新: 2026-06-22。参照CR: 2026-04-17 版。
...
参照CR: 領域=CR400、ターン構造=CR500、唱える/起動/誘発/静的/解決=CR601/602/603/604/608、
効果=CR609-616、イベント=CR700.4、ターンベース処理=CR703、状態起因処理(SBA)=CR704、
能力4分類=CR113.3、継続効果の層=CR613。
...
優先権取得前に自動適用(トグルで助言化)。**新 `src/engine/turn.ts`**。
...
- **誘発型(CR603)**: trigger condition(event)+ effect。**明示イベントストリームを新設**し、
  ...
  それを購読(observer: self/you/opponent/any)。介在条件(CR603.4 intervening-if)・
...
- 誘発はイベントで自動発火→スタック。効果が `auto` なら自動実行、`guided` はプロンプト、
...
### 契約ゲート(最初・spec承認必須)
- **M-CONTRACT**: `CLAUDE.md` L35 緩和の承認 + `engine-spec.md` に新章「ルール基盤+文法コンパイラ」
  追記(GameObject/有効特性/イベント/プレイヤー別ゾーンの型・新不変条件 I13+・スコープ境界・
  前方互換)。*契約のみ、エンジンコードなし*。
...
**依存関係**: M-CONTRACT → S-EVENTS / S-LAYERS(並行可)→ S-ZONES → S-ABILITY+DUMMY(events必須)
→ S-CONTINUOUS。C-GRAMMAR/C-BIND は events+layers 成立後に着手。C-COVERAGE は随時。A-LOOP は
S-ABILITY 後。
