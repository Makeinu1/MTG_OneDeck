...
The intended result is a compact, machine-checkable SOW envelope that makes delegated work precise enough to execute and audit while remaining subordinate to M0–M3/M3.1.
...
- **M3 / M3.1 — Verification Harness + Hardening** owns candidate-relative evidence binding, coverage invalidation, manual-evidence validation, impact selection, execution selection and verification freshness.
...
- an M3/M3.1 verification result;
- a release authorization;
- a persistent permission token;
- a status tracker;
- a roadmap.

If a Work Order conflicts with M0–M3/M3.1 authority, the Work Order is invalid or requires escalation. The Work Order never wins by being newer.
...
Those are M3/M3.1 outputs for an exact candidate.
...
- must be verifiable;
- may reference Acceptance IDs / M3.1 candidate verification;
...
- M3.1 candidate verification has no blocking obligation for the exact candidate;
...
7. M3.1 reports MANUAL_REQUIRED / DEFERRED / UNBOUND / UNKNOWN_COVERAGE that cannot be resolved within the declared work;
...
1. M3.1 determines semantic/evidence impact for exact base/head;
...
4. M3.1 verification state for the current candidate;
...
- required verification is established or explicitly blocked/deferred according to M3/M3.1;
...
For staleness, only an authority-source / ownership change to a referenced or protected semantic invalidates the SOW assumption. A semantic obligation becoming impacted because implementation or evidence changed is **not** by itself a change to semantic authority; it remains ordinary M3/M3.1 verification input.
...
- evidence result/freshness remains M3/M3.1-owned;
...
Can `doneWhen` or a SOW field store PASS/freshness and bypass M3/M3.1?
