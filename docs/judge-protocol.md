...
| 判断の種類 | 格上げ先 |
|---|---|
| 凍結判定・契約(spec の意味変更)承認・アーキ判断 | 判定者が決定・再オーナー化を保持。生の分析が要れば **GPT-5.6 を助言者照会**(冷たい別セッション・助言のみ・盤面/契約は変えない)。**Opus 4.8 召喚は任意**(助言で足りなければその判断のセッションだけ)|
...
CLAUDE.md「判定者ラダー」と「トークン経済」の表が正本(本文書では重複させない)。要点のみ: **7/8 以降の既定は J3 Sonnet 5 が判定者席に常駐**し、§7 の表に該当する判断は判定者が決定・再オーナー化を保持したうえで、生の分析が要れば GPT-5.6 を助言者照会する(実装者 GPT-5.6 は全 Claude ティアより高性能かつ Fable より低トークン=2026-07-15 ユーザー裁定。**Opus 4.8 召喚は任意**=助言で足りなければ)。機械作業(抽出・草稿・Tier-1・ship)+難草稿+助言は Codex(GPT-5.6)/Haiku/Sonnet サブエージェントへ。判定者が raw ソース精読・diff 行読み・契約初稿の自筆を始めたら委譲漏れのシグナル。
