...
**status**: 契約(判定者専有。実装エージェントは変更禁止・読み参照は推奨)
**作成**: 2026-07-09 Fable 5 引き継ぎセッション(ユーザー命令: デザインの仕事をFableが引き継ぎ、完成版までの作業を下位モデルで回せるよう徹底的に計画せよ)
**性格**: `docs/judge-protocol.md` のD-トラック版。ここに書いてある判断は**後継判定者(J3 Sonnet 常駐/J2 Opus 召喚)の地力を使わずそのまま適用してよい**。デザインの価値判断は本書とモックv4に先払い済み——後継の仕事は**照合と検証**であり、再設計ではない。

> **実行停止メモ(2026-07-12・J0草稿)**: D0/D1/D2/D3/D5出荷後、D4未実装のPC版に明確な
...
> D4分割はJ2、優先順位はユーザー、J0変更は復帰判定者の再オーナー化が必要。
