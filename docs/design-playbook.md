...
>
> **SUPERSEDED(2026-07-26・ユーザー裁定)**: 本書の D6「連鎖感」・D7「ループBGMは作らない」・
> §5.1/§5.2・§7のD6/D7完成条件は**実行禁止**。議論の結果、通常操作を履歴で盛らず、
> 同じ意味へ同じ手触りを返し、統率者キャストだけを固有儀式とする方針へ変更した。
> 新正本=`docs/audio-visual-contract.md`、HOW=`docs/ui-architecture-v2.md` §7、
> VERIFY=`docs/acceptance.md` M-AV。下位LLMは本書の旧カードを復活・部分流用しない。
...
- **1セッション=1スライス**。D0〜D5の記述は履歴。**D6/D7は2026-07-26にsupersededとなり実行禁止**。新AVスライスは `docs/audio-visual-contract.md` を使う。
...
| 検証の手順 | 本書§2 |
| 現行の音・意味イベント・BPM同期 | `docs/audio-visual-contract.md`。旧D6/D7とvision-sourcesを直接実装しない |
...
### D5 祝祭感(L1-L3中核) 【HISTORICAL SHIPPED CONTRACT・音/発火条件はSUPERSEDED】

> 以下は既存D5実装が作られた時点の記録であり、新規実装指示ではない。特にprimary/draw/resolveへの直接音・ハプティクス、連続回数による圧縮は現在の意味イベント契約ではない。撤去・移行対象は `docs/audio-visual-contract.md` §2・§9を使う。

**(a) ブリーフ**
```
目的: 4種モーション+ハプティクス+オプトイン音=Arenaとの体感差の中核(design-system §7のL1-L3)
スコープ: 4種のみ(これ以外の装飾モーション追加は禁止=design-system §9)。本行の旧DEFER先D6/D7はsuperseded。現行音・イベント境界は `docs/audio-visual-contract.md`。
...
### D6 連鎖感+演出レベル制御 【SUPERSEDED 2026-07-26・実行禁止】

旧カードの所在は§5.1。**正本ではなく、全文実行・部分流用とも禁止**。モックv4の場面4/5も新AVの視覚根拠にしない。

### D7 音+セッション演出 【SUPERSEDED 2026-07-26・実行禁止】

旧カードの所在は§5.2。**正本ではなく、全文実行・部分流用とも禁止**。モックv4場面1も新AVの視覚根拠にしない。

### D8 生きた背景(AmbientLayer)【HISTORICAL SHIPPED CONTRACT・テンポ/戦闘分岐はSUPERSEDED】

以下は既存D8実装の来歴であり、新規実装指示ではない。スコープ: design-system §8a の全実装——純関数 `src/components/game/ambientMotion.ts`(ビート周期/トグル状態/星・墨の決定的配置)+ `AmbientBackdrop.tsx`(`GameScreen` の `TabletopSurface` 直後に mount・`pointer-events:none`・`aria-hidden`)+ `game.css` §8a keyframes + `--ambient-*` トークン(`src/ui/tokens.css`)+ ThumbZone メニュー「背景モーション」トグル(既定 ON・`localStorage: mtg-onedeck:ambient-motion`・§7b 音トグルと同パターン)。
当時の視覚正本 = `research/design/mockups/ambient-motion.html`(**v4.3 形状確定**)。モック専用要素(速度スライダー×0.4-3・注釈・`.chrome`)は製品に持ち込まない。当時の固定テンポ裁定はsupersededであり、BGM中の現在正本はTrackManifest transport。
旧DEFER記録: 当時は音との同期(D7)・イベント層の追加演出を未実装とした。現在は固定テンポ・戦闘加速を含め `docs/audio-visual-contract.md` のAV0〜AV4へ置換済み。**フェイズ進行は既存 `TransitionCue` のみ=変更禁止**(ユーザー裁定)。
...
| **J3** | 既存トークン内の割当変更・アイコン追加(§6の20個枠内)・モックとの軽微乖離の記録承認・BoardShelf枚数閾値の±20%調整 | 新トークン・IA変更(バンド構成/行構成)・コンポーネント分割の変更・旧連鎖ヒューリスティックの調整/復活 |
...
## §5 D6/D7 旧契約ドラフト【SUPERSEDED・全文実行禁止】

### 5.1 D6 連鎖感+演出レベル制御【historical only】
...
### 5.2 D7 音+セッション演出【historical only】
...
2. D6/D7の着手goは撤回済み。新AVは `docs/audio-visual-contract.md` §8の人間ゲートを使う。
3. 旧「D6/D7 shipで完成」の定義は無効。通常操作の予測可能性・疲れにくさ・統率者の固有儀式をユーザーが最終裁定する。
